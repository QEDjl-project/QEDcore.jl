...
function _rnd_boost_params(rng, T = Float64)
    xyz = Tuple(rand(rng, T, 3))
    xyz = @. (2 * xyz - 1) / sqrt(T(3))
    return xyz
end

function _generate_onshell_two_body_moms(rng, masses, sqrt_s)
    T = promote_type(eltype(masses), typeof(sqrt_s))

    rnd_boost = Boost(BetaVector(_rnd_boost_params(rng, T)...))
    p1_restframe = SFourMomentum{T}(masses[1], 0, 0, 0)

    E2_restframe = (sqrt_s^2 - sum(masses .^ 2)) / (2 * masses[1])
    rho2 = sqrt(E2_restframe^2 - masses[2]^2)
    cth2 = 2 * rand(rng, T) - one(sqrt_s)
    sth2 = sqrt(one(cth2) - cth2^2)

    phi2 = 2 * T(pi) * rand(rng, T)
    sphi2, cphi2 = sincos(phi2)

    p2_restframe = SFourMomentum{T}(
